...
const navItems = [
  { href: "/shipper/dashboard", label: "Dashboard", icon: "squares" },
  { href: "/shipper/create-delivery", label: "New delivery", icon: "plus" },
  { href: "/shipper/history", label: "History", icon: "clock" },
  { href: "/shipper/settings", label: "Settings", icon: "gear" },
];
...
function NavIcon({ type, className }: { type: string; className?: string }) {
  ...
  switch (type) {
    ...
    case "plus":
      return (<svg className={c} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>);
    case "clock":
      return (<svg className={c} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>);
    case "gear":
      ...
      <nav className="fixed left-0 right-0 top-0 z-50 border-b border-black/5 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
          <div className="flex items-center gap-8">
            <Link href="/shipper/dashboard" className="text-[15px] font-semibold tracking-tight text-[#1d1d1f]">
              Sprinter Cargo
            </Link>
